...
def Execute(op, obj):
    ...
    try:
        ...
        stockpaths = [
            {"depths": p["depths"], "path2d": convertTo2d(p["edges"])} for p in op.stockPathArray
        ]
        ...
        # FIXME: Sorting already guaranteed by how these lists are created?
        ...
        if inputStateChanged or adaptiveResults is None:
            ...
            for rdict in regionOps:
                ...
                stockpath = None
                for s in stockpaths:
                    if rdict["startdepth"] in s["depths"]:
                        stockpath = s
                        break

                rdict["toolpaths"] = a2d.Execute(stockpath["path2d"], path2d, progressFn)
            ...
            if obj.OrderCutsByRegion:
                ...
                def addToCutList(regionId):
                    for k in outsidePathArray2dDepthTuples + insidePathArray2dDepthTuples:
                        if k[1]["id"] == regionId:
                            # Should never happen
                            if k in cutlist:
                                return
...
def projectFacesToXY(faces, tolerance=1e-10):
    ...
    for f in faces:
        ...
        if type(f.Surface) in [Part.Cone, Part.Cylinder, Part.Sphere]:
            ...
            wires = DraftGeomUtils.findWires(
                [e for e in f.Edges if not e.isSeam(f) and e.Length > tolerance]
            )
...
def _working_edge_helper_roughing(op, obj, depths):
    ...
    for depth in depths:
        ...
        stockface = _get_solid_projection(op.stock.Shape, depth)
        above_refined = _get_solid_projection(shp, depth)
        ...
        # FIXME: jumping through some hoops here to remove internal holes from
        # all of the "outside" faces- seems like there's probably a one-shot
        # command for that?
        regions = [
            Part.makeFace(TechDraw.findShapeOutline(f, 1, projdir)) for f in above_refined.Faces
        ]
        if regions:
            regions = regions[0].fuse(regions[1:])
        else:
            regions = Part.Shape()
        ...
        # FIXME: This will break if shp has more than one disjoint piece!
...
def _working_edge_helper_manual(op, obj, depths):
    ...
    # FIXME: How to handle multiple s_get_solid_projectionhapes? Wrap entire thing in a "for m in op.model"? Fuse them all together?
    shp = op.model[0].Shape
    ...
    projface = Path.Geom.makeBoundBoxFace(shp.BoundBox, offset=1, zHeight=0)
    projdir = FreeCAD.Vector(0, 0, 1)
    ...
    # FIXME: Any reason to cut the selected regions with the stock at the given depth?
    # NOTE: Maybe an (unsubstantiated) reason against it: for holes in the stock,
    # allows plunging into internal pockets, that might be lost if we cropped
    # the area to machine to the stock?
    for depth in depths:
...
def _get_working_edges(op, obj):
    ...
    # FIXME: Adjust how finish_step is used! Maybe remove entirely?
    ...
    # FIXME: Unclear that there's any value in merging these, as opposed to the
    # value of being able to just look up via stockDict[depth]
    stockProjections = ((d, _get_solid_projection(op.stock.Shape, d)) for d in depths)

    stock_regions = []
    for d, region in stockProjections:
        if region.isNull():
            continue
        # No stock_region entries OR this depth is different than the last -> add
        if not stock_regions or region.cut(stock_regions[-1]["region"]).Wires:
            stock_regions.append({"region": region, "depths": [d]})
            continue
        # Same as previous -> add new depth to the list
        else:
            stock_regions[-1]["depths"].append(d)
    ...
    # FIXME: Or just use the "region" field? That might introduce overhead we
    # don't really want/need- eg, not letting memory be garbage collected?
    idnumber = 0
    ...
    # FIXME: This can almost certainly be done more elegantly
    for rx in inside_regions:
        ...
        for ry in [k for k in outside_regions]:
            ...
            if rx["region"].Area == rx["region"].cut(ry["region"]).Area:
    ...
    # FIXME: Could short-circuit logic if there's only one stock entry, or if
    # all of the regions depths are in a single stock region (sufficient, not
    # necessary)
    for r in inside_regions:
        depths = sorted(r["depths"], reverse=True)
        stockregion = None
        for s in stock_regions:
            if depths[0] in s["depths"]:
                stockregion = s["region"]
                break

        rc = r["region"].cut(stockregion)
        ...
        for d in depths[1:]:
            ...
            # the existing region.
            if r["region"].cut(stockregion).cut(rc).Wires:
                newregion = {
                    "id": idnumber,
                ...
                inside_regions.append(newregion)
                idnumber += 1
                continue
    ...
    # FIXME: Might be able to merge these somewhat nicely? Basically the same,
    # just inverses...
    for r in outside_regions:
        ...
        for d in depths[1:]:
            ...
            # The new region gets all of the children, and becomes a child of
            # the existing region.
            if stockregion.cut(r["region"]).cut(sc).Wires:
                newregion = {
                    "id": idnumber,
                    "depths": [k for k in depths if k not in parentdepths],
                    "region": r["region"],
                ...
                # When the new region is processed at the end, we'll effectively
                # recurse and handle splitting that new region if required
                outside_regions.append(newregion)
                idnumber += 1
                continue
            # If we didn't split at this depth, the parent will keep "control"
            # of this depth
    ...
    # FIXME: We still end up fully cutting areas that can be accessed from outside
    # the stock. Not clear why- maybe want to AND inside regions with the stock,
    # or maybe need to somehow make regions adjacent to holes in the stock
    # "outside" regions
    ...
    stock_discretized = list()
    for rdict in stock_regions:
        discretizedEdges = list()
        for a in rdict["region"].Faces:
            for w in a.Wires:
                discretizedEdges.append([discretize(w)])
        stock_discretized.append(
            {
                "edges": discretizedEdges,
                "depths": rdict["depths"],
            }
