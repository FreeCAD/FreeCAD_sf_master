...
T* Document::addObject(const char* pObjectName, bool isNew, const char* viewType, bool isPartial)
{
    ...
    auto namespaceAndClass = boost::typeindex::type_id_with_cvr<T>().pretty_name();
    return static_cast<T*>(addObject(namespaceAndClass.c_str(), pObjectName, isNew, viewType, isPartial));
}
